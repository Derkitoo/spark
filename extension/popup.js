...
async function saveIdea(session, context) {
  const title = app.querySelector(".title").value.trim(), note = app.querySelector(".note").value.trim(), category = app.querySelector("select").value, tagsRaw = app.querySelector(".tags")?.value || "";
  if (!title) throw new Error("Ajoute un titre à l’idée.");
  const tags = Array.from(new Set(tagsRaw.split(/[\s,]+/).map(t => t.trim().replace(/^#/, "")).filter(Boolean)));
  const content = [note, context.selection ? `Extrait : “${context.selection}”` : "", context.url ? `Source : ${context.url}` : ""].filter(Boolean).join("\n\n");
  const response = await fetch(`${SUPABASE_URL}/rest/v1/ideas`, { method: "POST", headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${session.access_token}`, "Content-Type": "application/json", Prefer: "return=minimal" }, body: JSON.stringify({ user_id: session.user.id, title, content, category, tags, status: "Capturée" }) });
...
async function renderCapture(session) {
  ...
  app.innerHTML = `<header class="top"><div class="wordmark"><span class="logo">S</span>Spark</div><button class="account" title="Se déconnecter">${displayName} · quitter</button></header><section class="capture"><span class="eyebrow">QUICK CAPTURE</span><h1>Garde cette étincelle.</h1><div class="source"><strong>${escapeHtml(context.title)}</strong><small>${escapeHtml(context.url)}</small></div>${context.selection ? `<div class="selected">“${escapeHtml(context.selection)}”</div>` : ""}<textarea class="title" aria-label="Titre">${escapeHtml(context.selection ? context.selection.slice(0, 180) : context.title)}</textarea><textarea class="note" placeholder="Ajoute une note personnelle…"></textarea><input class="tags" placeholder="Tags (ex: web, idée, urgent)…" style="width:100%;margin-bottom:8px;border:1px solid #d4d9d1;border-radius:6px;padding:6px 9px;font-size:11px;outline:0;" /><div class="row"><select><option>Personnel</option><option>Projet</option><option>Créativité</option><option>Travail</option><option>À partager</option></select><button class="save">Enregistrer ↗</button></div><div class="status"></div><a class="open" href="${APP_URL}" target="_blank">Ouvrir ma bibliothèque Spark</a></section>`;
